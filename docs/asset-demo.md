...
`traverse` estimates relative passage potential across a gridded
landscape between source and destination nodes. This demonstration uses
the package’s example rasters to show the full workflow: define a
scientific target domain, expand it for computation, place terminals
outside the target, and mask the result back to the area being
interpreted.

## Landscape inputs

The **target domain** is the set of cells for which passage will
ultimately be interpreted. The **movement surface** describes relative
suitability or preference across that landscape; higher values indicate
easier or more preferred movement. In `template.tif`, finite `0` values
mark included cells and `NA` marks cells outside the target. The zero is
a mask value only; it is not a movement cost. In `surface.tif`, larger
continuous values indicate greater suitability.
